...
| [Application contracts](analysis-accuracy.md) | Reconciles code routes and OpenAPI auth, verifies declared allow/deny/tenant test IDs, and finds exact manifest-listed vulnerable calls | security/architecture | Bundled AST plus retained JSON contracts and governed test artifacts; dynamic dispatch and exploit preconditions remain explicit limitations |
| [Code and architecture health](analysis-accuracy.md) | Adds cognitive complexity, large responsibility surfaces, parameter coupling, duplicate functions, local dependency cycles/fan-out, bounded Git co-change, and finding/change hotspots | quality/architecture | Bundled AST plus already-sealed bounded Git history; co-change remains a review signal, not a defect claim |
