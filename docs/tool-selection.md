...
| [mypy](https://mypy.readthedocs.io/) | Finds invalid type contracts and ambiguous error paths that pattern SAST does not model | quality | Does not follow imports or use target site packages |
...
| [Sensitive-data exposure synthesis](data-exposure.md) | Joins Semgrep/Pysa/CodeQL evidence to logs, telemetry, request collections, URL queries, client errors, SDK configuration, reachability, and coverage context | security | Bundled AST inventory and immutable rules; no target imports, execution, SDK calls, or network access |
