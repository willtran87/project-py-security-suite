...
| [Cross-domain assurance](domain-assurance.md) | Detects specialized repository domains and reconciles repository-owned objectives with exact enforcement files, complete governed artifacts, and passing source-bound tests | security/architecture/governance | Bundled bounded AST/path analysis and retained artifacts; signals establish applicability only, while native and companion tools retain responsibility for domain-specific detection and runtime proof |
| [LLM adversarial planning](llm-adversarial-testing.md) | Converts existing evidence into provider-neutral attack hypotheses and confined generated-test handoffs | security/testing/architecture/quality | Core planning is deterministic and model-free; a separately authorized companion validates schema-bound proposals and deterministic tool evidence, while an LLM can never self-confirm a finding |
| [Industry standards and benchmarks](industry-standards-benchmarks.md) | Makes standards scope and measured detector performance explicit instead of inferring assurance from tool count | governance/security/quality/architecture | Core crosswalk, control assessment, OSCAL export, and scorecard joins are offline; intentionally vulnerable OWASP/NIST targets and LLM evaluations run only in separately authorized disposable lanes with pinned corpora |
