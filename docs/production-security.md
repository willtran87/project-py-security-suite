...
| Industry benchmark execution | Registry-bound scoring for 282 families with 212 maintained adapters, eleven typed protocols, and 116 semantic evidence integrations | Disposable authorized lanes now include ETSI AI-lifecycle conformance, nested evaluator-harness escape containment, FIASSE securability mutation, NSA-informed MCP identity/context/serialization/teardown cases, and the existing cloud, GenAI, CHECK, AIUC-1, IoT, resilience, regulated-sector, formal-method, supply-chain, vulnerable-application, fuzzing and architecture suites. The scanner validates evidence and scorecards but does not execute active targets inside the source-scan boundary. |
...
`security-requirements-coverage.json` schema 1.1 records pinned ASVS 5.0.0,
MASVS 2.1.0, TCASVS 5.0.1, and AISVS 1.0 catalog metadata and mapped evidence.
Organization policy 1.1 may add further source-revision- and digest-pinned
verification catalogs; AI applicability requires AISVS. Full completion requires
