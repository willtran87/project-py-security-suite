...
| Industry benchmark registry and regression | Yes; external vulnerable targets use disposable companion lanes | Pinned corpus digests, approved corpus authority, replay protection, verified source-bound reports, and optional approved prior scorecard | Eight OWASP/NIST/LLM/holdout families, threshold decisions, aggregate scorecard, and explicit comparable regression deltas |
| Standards control assessment and OSCAL | Yes | Strict repository policy naming known catalog/control identities and complete governed evidence artifacts | 19-catalog crosswalk, satisfied/gap/not-applicable control status, production enforcement, and OSCAL 1.1.2 assessment-results interchange |
