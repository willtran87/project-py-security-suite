...
| Static risk-route synthesis | Yes | Reachability entry points, Graphify file/reverse-test edges, normalized findings, sensitive-data sink surfaces, structural synthesis, source inventory, file/diff coverage, bounded JUnit/Hypothesis/Schemathesis cases, and CODEOWNERS | Stable bounded entry-to-target routes, exact relation/file sequences, shared transit/target control points, direct/transitive validation campaigns with observed execution, source-revision coherence, factorized review scores, hotspot coverage alignment, owner queues, consolidated actions, runtime context, and explicit unrouted model gaps; never treated as proof of exploitability or leakage |
