...
| Declared architecture policy | Yes | Local Python imports/calls, unified entry-point evidence, dynamic imports, optional approved edge baseline, and strict native JSON or Tach dependency policy | Exact high-confidence layer/forbidden/undeclared-edge violations kept separate from syntactic calls, unresolved dynamic-import gaps, cycles, fan-out, hubs, instability, co-change, and new-edge heuristics |
