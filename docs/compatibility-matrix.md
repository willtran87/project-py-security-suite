...
| Cross-tool evidence fusion | Yes | Normalized findings, tool status, source/artifact SBOMs, graph, reachability, coverage, complexity, structural synthesis, CODEOWNERS, source inventory, artifact manifest, scanner fix records, and approved KEV/EPSS/VEX | Per-finding review reasons, semantic/cross-stage corroboration, package lineage drift, digest agreement, compound hotspots, alias-aware advisory decisions with attributed fix candidates, exposure ownership/test/change-risk feedback, SDK-package disclosure-boundary joins, and evidence-lane gaps |
| Advisory validation handoff | Yes | Exact Graphify import/reverse-test topology, retained file coverage, CODEOWNERS-derived finding ownership, alias-aware advisory clusters, and remediation context | Direct/transitive focused tests with confidence, import-path owners, low-coverage validation gaps, and one closure-plan item per distinct advisory while all scanner observations remain auditable |
...
- Coverage produces an overall threshold finding plus at most ten lowest-file
  ...
  bounded metadata in `junit-summary.json`.
