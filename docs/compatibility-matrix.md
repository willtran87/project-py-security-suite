...
| Static risk-route synthesis | Yes | Reachability entry points/runtime observations, Graphify file/reverse-test edges, normalized findings, sensitive-data sink surfaces, structural change-risk synthesis, source inventory, file/diff coverage, bounded JUnit/Hypothesis/Schemathesis cases, and CODEOWNERS | Stable bounded entry-to-target routes, exact relation/file sequences, shared transit/target control points, direct/transitive validation campaigns, cross-campaign shared-test bottlenecks and sole-test dependencies, observed execution, source-revision coherence, changed-line/runtime-aware factorized review scores, owner queues, consolidated actions, and explicit unrouted model gaps; never treated as proof of exploitability, leakage, or test independence |
| Advisory validation handoff | Yes | Exact Graphify import/reverse-test topology, retained file coverage, CODEOWNERS-derived finding ownership, bounded JUnit/Hypothesis/Schemathesis cases, alias-aware advisory clusters, and remediation context | Direct/transitive focused tests with confidence, exact current execution status, import-path owners, low-coverage validation gaps, and one closure-plan item per distinct advisory while all scanner observations remain auditable |
