...
| Industry benchmark registry and regression | Yes; external vulnerable targets use disposable companion lanes | Pinned corpus digests and revisions, approved authority, replay protection, validated time, verified report checksum, complete confusion matrix, and optional prior scorecard | Nine OWASP/NIST/LLM/AI/holdout families, per-family runner contracts, Wilson confidence intervals, Youden's J, threshold decisions, and comparable regression deltas |
| Standards controls, procedures, prioritization, and OSCAL | Yes | Strict policy 1.1 naming known controls and procedures, complete evidence, explicit execution authorization, and source-provided prioritization data | 39-reference crosswalk; control and procedure outcomes; CVSS v4/SSVC without fabricated scores; and seven official-schema-valid OSCAL 1.2.2 lifecycle models |
