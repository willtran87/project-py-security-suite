...
| Industry benchmark registry and regression | Yes; active targets use disposable companion lanes | Pinned inputs, signed acceptance, source/subject binding, independent replay, deterministic resource budgets, shell-free OCI execution, negative controls and domain-specific claim schemas | 282 families, 212 maintained adapters, eleven protocols and 116 semantic integrations spanning ETSI AI lifecycle conformance, agentic evaluator containment, FIASSE securability, NSA-informed MCP, CIS cloud/Docker, GenAI red teaming, AI Verify/Moonshot, CHECK, AIUC-1, IoT, resilience, regulated sectors, formal methods, empirical calibration, supply chain and architecture |
| Standards controls, procedures, prioritization, and OSCAL | Yes | Strict policy 1.3, requirements coverage 1.1, fail-closed catalog integrity, explicit authorization, signed publisher observations, human promotion, assessor credentials and bounded certification claims | 663-reference crosswalk, 71-item watchlist and 233 selectable profiles; AISVS-capable extensible requirement catalogs; seven OSCAL 1.2.2 lifecycle models; drafts, research corpora, alpha material and licensed content cannot silently become normative |
...
| Semgrep CE | Organization-defined structural and taint rules, including credential fields, private fields, precise request collections, runtime/environment-state dumps, logs, telemetry, direct URL interpolation and query parameters, raw client errors, risky SDK/configuration capture, unauthenticated gRPC channels, and dynamic Psycopg SQL composition | Local immutable rules; generic configuration and Python analysis; stable governed IDs for digest-bound framework canaries; metrics and version checks disabled | standard and broader | Yes |
