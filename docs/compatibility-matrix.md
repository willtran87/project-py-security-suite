...
| Industry benchmark registry and regression | Yes; external vulnerable targets use disposable companion lanes | Pinned corpus digests and revisions, approved protocol-specific acceptance criteria, subject-bound Ed25519 evidence, consumed replay receipts, validated time, digest-pinned executables, shell-free bounded execution, native read-only/capability-free/no-network OCI mode, signed SBOM/provenance, and negative controls | 99 families spanning OWASP/NIST/LLM/AI/identity/crypto/polyglot/IaC/container/SBOM/SCITT/fuzzing/agentic/architecture/API/telemetry/resilience/STIG/OT/forensics/accessibility plus CFReDS/CFTT, ACT Rules, DroidBench, Ghera, SecBench.js, Chaos Mesh/Litmus, Sonobuoy, CIS/SCAP, C2SP Wycheproof, and TIBER-EU; nine executable scoring protocols and maintained adapter specifications |
| Standards controls, procedures, prioritization, and OSCAL | Yes | Strict policy 1.2 selecting bounded assurance packs or naming custom controls/procedures, complete evidence, explicit execution authorization, quarantined HTTPS publisher observations, signed snapshots, JSON/XML/HTML/PDF/text semantic change analysis, affected-control mapping, human promotion approval, verified assessor credentials, and source-provided prioritization data; policy 1.0/1.1 remain readable | 292-reference crosswalk plus publisher monitor, lifecycle ledger, and publication watchlist; 80 general and conditional assurance packs; governed lifecycle, architecture, process-capability, temporal-prioritization, maturity, fail-closed interoperability, and external-conformity evidence; CVSS v4/SSVC without fabricated scores; 99 benchmark contracts; and seven official-schema-valid OSCAL 1.2.2 lifecycle models |
