...
| Static risk-route synthesis | Yes | Reachability entry points, Graphify file edges, normalized findings, sensitive-data sink surfaces, structural synthesis, coverage/diff coverage, focused-test execution, and CODEOWNERS | Stable bounded entry-to-target routes, exact relation/file sequences, shared transit/target control points, owner queues, consolidated validation actions, runtime context, and explicit unrouted model gaps; never treated as proof of exploitability or leakage |
...
| JUnit evidence | Automated test outcomes and exact selected-test execution | Validates bounded XML metadata; retains an output-free case/file/result ledger and drops output and failure bodies | quality, repo, comprehensive, production, release | Yes |
