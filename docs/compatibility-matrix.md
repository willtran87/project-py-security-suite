...
| Static risk-route synthesis | Yes | Reachability entry points/runtime observations, Graphify file/reverse-test/import edges, normalized findings, alias-aware dependency advisory/remediation clusters, sensitive-data sink surfaces, structural change-risk synthesis, source inventory, file/diff coverage, bounded JUnit/Hypothesis/Schemathesis cases, and CODEOWNERS | Stable bounded entry-to-finding/sink/dependency-importer routes, citation-bearing threat/fix context, exact relation/file sequences, shared transit/target control points, direct/transitive validation campaigns, cross-campaign shared-test bottlenecks and sole-test dependencies, observed execution, source-revision coherence, changed-line/runtime-aware factorized review scores, owner queues, consolidated actions, and explicit unrouted model gaps; never treated as proof of vulnerable-function invocation, exploitability, leakage, or test independence |
