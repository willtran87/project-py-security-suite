...
| Static risk routes | [1.0](../src/py_security_suite/schemas/risk-paths.schema.json) | Bounded entry-point routes to findings, review-worthy sensitive sinks, and exact advisory importer paths with citation/fix/threat context, runtime, ownership, shared control-point convergence, graph-selected validation campaigns, cross-campaign shared-test bottlenecks, source-revision coherence, changed-line/runtime-aware review scoring, exact test/coverage alignment, owner queues, and explicit unrouted-target context |
