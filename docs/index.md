...
| Static risk routes | [1.0](../src/py_security_suite/schemas/risk-paths.schema.json) | Bounded entry-point routes to findings, review-worthy sensitive sinks, and exact advisory importer paths; fail-closed source/artifact package lifecycle; exact-path sensitive-boundary/advisory intersections; citation/fix/threat context; runtime, ownership, shared controls, graph-selected campaigns, shared-test bottlenecks, source-revision coherence, validation alignment, owner queues, and explicit model gaps |
