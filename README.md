...
| Industry assurance | 663 versioned references, 71 quarantined watch items, 233 selectable profiles, 282 governed benchmark families, 212 maintained adapters, eleven typed protocols, and 116 suite-owned semantic integrations. Coverage includes ETSI EN 304 223 AI lifecycle cybersecurity, OWASP LLM Top 10 2026 and AISVS requirements-as-code, Five Eyes agentic adoption, NSA-informed MCP security, MAESTRO cross-layer threat modeling, FIASSE securability, evaluator-harness escape containment, CIS cloud/Docker, GenAI red teaming, AI Verify/Moonshot, CHECK, AIUC-1, IoT, resilience and regulated sectors. Every active benchmark is subject-bound, independently replayed, negative-tested and isolated; CRA draft standards, experimental AIVSS scoring, research datasets, alpha material, licensed content and external-certification boundaries remain explicit. |
...
| Suite assurance | Exact 155-boundary dependency graph with one frozen seven-module bootstrap SCC; zero-slack file/function/decision debt ceilings that must tighten with every reduction; Mypy over the full source surface plus a strict 205-module production contract, and a pinned strict Pyright gate over 36 production trust-boundary modules and 12 assurance scripts; 50 strengthened security-critical coverage ratchets; six performance workloads including the real code-health/static-architecture pipeline; AST-verified live-test policy plus complete JUnit engine/role matrix evidence; and aggregate-only governed effectiveness statistics that preserve holdout secrecy while supporting release thresholds |
...
flowchart LR
    ...
    subgraph Boundary["Externally isolated execution boundary"]
        ...
        Benchmarks["282 pinned benchmark families<br/>application | identity | AI | safety | resilience | operations | calibration"] --> BenchmarkLane["212 maintained benchmark adapters<br/>authorized disposable/external lanes<br/>11 typed scoring protocols"]
        BenchmarkLane --> ResilienceLanes["Recovery + sanitization + OT + crisis<br/>provider evaluation + ICT risk + crosswalk + LNG/EV"]
        ResilienceLanes --> SemanticGate["116 suite-owned semantic integrations<br/>source + subject binding | domain oracles<br/>negative controls + independent replay"]
...
|-- security-requirements-coverage.json # extensible ASVS/MASVS/TCASVS/AISVS evidence crosswalk and gaps
